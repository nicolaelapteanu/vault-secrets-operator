...
func (r *VaultSecretReconciler) restartResource(ctx context.Context, restartSpec *v1alpha1.RestartOnChangeSpec, namespace string, instance *ricobergerdev1alpha1.VaultSecret) error {
	...
	case "Deployment":
		...
		deployment.Spec.Template.Annotations["vaultsecret/restartedAt"] = time.Now().Format(time.RFC3339)
	...
	case "StatefulSet":
		...
		statefulset.Spec.Template.Annotations["vaultsecret/restartedAt"] = time.Now().Format(time.RFC3339)
